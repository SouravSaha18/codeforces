...
int main()
{
    ...
    wl(t--)
    {
        ...
        lp1(i,0,n)
        {
            lp1(j,0,n)
            {
                ch= max(vv[i][j], max(vv[j][n-i-1], max(vv[n-j-1][i], vv[n-i-1][n-j-1])));
                cc=cc+(ch-vv[i][j])+(ch-vv[j][n-i-1])+(ch-vv[n-j-1][i])+(ch-vv[n-i-1][n-j-1]);
                vv[i][j]=vv[j][n-i-1]=vv[n-j-1][i]=vv[n-i-1][n-j-1]=ch;
            }
        }
        cout<<cc<<endl;
    }
